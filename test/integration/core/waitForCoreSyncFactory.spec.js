...
describe('waitForCoreSyncFactory', function main() {
  ...
  it('shouldn\'t wait if Xazab Core in regtest mode without peers', async () => {
    thirdXazabCore = await startXazabCore();

    container = await createTestDIContainer(mongoDB, thirdXazabCore);

    waitForCoreSync = container.resolve('waitForCoreSync');

    await waitForCoreSync(() => {});

    const api = thirdXazabCore.getApi();
