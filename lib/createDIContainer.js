...
const Identifier = require('@xazab/dpp/lib/identifier/Identifier');
 ...
 * @param {string} options.XAZABPAY_CONTRACT_ID
 * @param {string} options.XAZABPAY_CONTRACT_BLOCK_HEIGHT
...
async function createDIContainer(options) {
  ...
  if (options.XAZABPAY_CONTRACT_ID && !options.XAZABPAY_CONTRACT_BLOCK_HEIGHT) {
    throw new Error('XAZABPAY_CONTRACT_BLOCK_HEIGHT must be set');
  }
  ...
  container.register({
    ...
    xazabContractId: asValue(
      options.XAZABPAY_CONTRACT_ID
        ? Identifier.from(options.XAZABPAY_CONTRACT_ID)
        : undefined,
    ),
    xazabContractBlockHeight: asValue(parseInt(options.XAZABPAY_CONTRACT_BLOCK_HEIGHT, 10)),
